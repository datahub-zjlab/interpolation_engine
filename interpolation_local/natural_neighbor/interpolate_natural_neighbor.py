"""
自然邻点插值本地实现，以SiO2为特征，使用有SiO2的点集构建插值点网格。
"""
...
gx, gy, img = interpolate.interpolate_to_grid(
    x, y, z, interp_type='natural_neighbor', hres=0.05)

# 导出插值结果到CSV文件，参考K2O_Na2O_region.csv。
df_nn = pd.DataFrame({'box_lon': gx.flatten(),
                      'box_lat': gy.flatten(), 'value': img.flatten()})
df_nn.to_csv('SiO2_natual_neighbor.csv', index=False)

# 画图
visualization.map(df, 'SiO2', df_nn)

# --------------------------------------------------
# 测速：不同分辨率，每组十次。
times_nn = defaultdict(list)
for hres in range(10, 0, -1):
    hres = hres/20
    for _ in range(10):
        start_t = time.time()
        gx, gy, img = interpolate.interpolate_to_grid(
            x, y, z, interp_type='natural_neighbor', hres=hres)
        end_t = time.time()
        grid_num = img.shape[0]*img.shape[1]
        print(
            f'natural neighbor interpolation takes {end_t-start_t:.2f}s, hres: {hres}, grid_num: {grid_num}')
        times_nn[(hres, grid_num)].append(end_t-start_t)

times = [(hres, gird_num, np.mean(t))
         for (hres, gird_num), t in times_nn.items()]
times = pd.DataFrame(times, columns=['hres', 'grid_num', 'time'])
print(times)
times.to_csv('times_natural_neighbor.csv')
